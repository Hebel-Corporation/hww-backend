...
def create_pairing_bonuses(new_member_account, upline, position:str):
    ...
    if upline.sponsor_account :
        create_pairing_bonuses(new_member_account,upline=upline.sponsor_account,position=upline.position)
...
def on_office_save(instance):
    ...
    if not instance_exists(instance):
        instance.office_code = get_new_company_id(instance,IdType.OFFICE)
...
def on_account_save(instance):
    ...
    if not instance_exists(instance):
        ...
        if instance.sponsor_account.get_children().filter(position="left").exists():
            instance.position = "right"

        instance.company_id =  get_new_company_id(instance, IdType.ACCOUNT)

        create_pairing_bonuses(instance,instance.sponsor_account,instance.position)
